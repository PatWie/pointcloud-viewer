...
// half angle, whic his the angle from one side of the cone to the ray in the
// center of the cone
//
struct cone_t final {
 public:
  glm::vec3 origin;
  float tan_half_angle;  // precomputed tangens of the half angle. Used to
                         // quickly get he width of the cone for a distance
  glm::vec3 direction;
  float inv_cos_half_angle;  // procumputed inverse of the cos of a half angle.
                             // As this code is imported from anothe rproject of
                             // mine, it's not used here. But it would be
                             // usefull, if we would introduce a cone/sphere
                             // intersectoin test
  ...
  // Returns the half angle (anjgle from the center_ray() to one side of the
  // cone)
  float half_angle() const;
  // Returns the width of a cone at the given distance t
  float cone_radius_at(float t) const;

  // Returns the closest ray within the cone (sharing the origin with the cone)
  // to the given point, which may or may not be within the cone.
