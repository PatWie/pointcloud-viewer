...
`format` helps formatting a string. For example
    ...
    format("answer to the life, universe and everything: ", 42, ". Deep inside,
you know it's true!")

`print` do the same, but they already prints the result to the console.
`print` do the same, but includes an std::endl at the end

`PRINT` is a macro printing an expression along with the expression itself. For
example PRINT(6*7) prints `6*7: 42`
*/
