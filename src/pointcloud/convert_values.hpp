...
from_float_normalized(float_t value) {
  ...
  return T(glm::clamp<float_t>(glm::round(value), 0,
...
from_float_normalized(float_t value) {
  ...
  return T(glm::clamp(glm::round(value), float_t(std::numeric_limits<T>::min()),
                      float_t(std::numeric_limits<T>::max())));
}
...
struct convert_component {
  static void convert_absolute(const void* source, void* target) {
    ...
    if (std::is_floating_point<t_in>::value &&
        !std::is_floating_point<t_out>::value)
      in_value = glm::round(in_value);

    in_value =
        glm::clamp<t_in>(in_value, t_in(std::numeric_limits<t_out>::min()),
  ...
  static void convert_normalized(const void* source, void* target) {
    ...
    write_value_to_buffer(
        target,
        from_float_normalized<t_out>(static_cast<float_t>(
            to_float_normalized<t_in>(read_value_from_buffer<t_in>(source)))));
  }
};
