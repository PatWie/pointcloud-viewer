...
DebugMesh::DebugMesh() : vertex_buffer(), num_vertices(0) {}

DebugMesh::DebugMesh(const vertex_t* vertices, int num_vertices)
    : vertex_buffer(int(sizeof(vertex_t)) * num_vertices,
                    gl::Buffer::UsageFlag::IMMUTABLE, vertices),
...
DebugMesh DebugMesh::trackball(glm::vec3 origin, float r) {
  Generator generator;
  int brightness = 0;
  Color axis_colors[3] = {color_palette::red[brightness],
  ...
  for (int dim = 0; dim < 3; ++dim) {
    ...
    generator.next_attribute.color =
        axis_colors[dim].with_saturation(Color(0xcc8080));
    generator.push_matrix(origin, target);
    generator.add_circle(r, 64);
    generator.pop_matrix();
  }
...
DebugMesh DebugMesh::grid(int repetition_per_side, float cell_size,
                          glm::vec3 color, glm::vec3 origin, glm::vec3 axis_1,
                          glm::vec3 axis_2) {
  Generator generator;
  generator.next_attribute.color = color;
...
DebugMesh DebugMesh::path(int path_length,
                          std::function<frame_t(int)> frame_for_index,
                          int selection) {
  if (path_length == 0) return DebugMesh();
  ...
  const glm::vec3 default_color =
      glm::mix(highlight_color, glm::vec3(color_palette::grey[3]), 0.5f);
...
DebugMesh DebugMesh::cone(cone_t cone) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    generator.add_vertex(glm::vec3(glm::cos(angle) * max_radius,
                                   glm::sin(angle) * max_radius, max_length));
  }
...
DebugMesh DebugMesh::highlighted_point(glm::vec3 coordinate, glm::vec3 color,
                                       float radius) {
  Generator generator;
...
DebugMeshRenderer::DebugMeshRenderer()
    : shader_object("debug_mesh_renderer"),
      vertex_array_object({Attribute(Attribute::Type::FLOAT, 3,
                                     DEBUG_MESH_VERTEX_BUFFER_BINDING),
  ...
  shader_object.AddShaderFromFile(
      gl::ShaderObject::ShaderType::VERTEX, "debug/debug_mesh.vs.glsl",
      format("#define DEBUG_MESH_VERTEX_ATTRIBUTE_LOCATION_POSITION ",
             DEBUG_MESH_VERTEX_ATTRIBUTE_LOCATION_POSITION, "\n",
             "#define DEBUG_MESH_VERTEX_ATTRIBUTE_LOCATION_COLOR ",
             DEBUG_MESH_VERTEX_ATTRIBUTE_LOCATION_COLOR, "\n"));
...
void DebugMeshRenderer::render(const DebugMesh& mesh) {
  ...
  mesh.vertex_buffer.BindVertexBuffer(
      DEBUG_MESH_VERTEX_BUFFER_BINDING, 0,
      GLsizei(vertex_array_object.GetVertexStride(
          DEBUG_MESH_VERTEX_BUFFER_BINDING)));
...
DebugMesh::Generator::Generator() { transformations.push(glm::mat4(1)); }
...
void DebugMesh::Generator::end_strip() {
  if (this->first_strip_vertex < vertices.length()) {
    ...
    // duplicating, because passing a reference to a value within the array
    // vertices, which might be modified with vertices.resize within addVertex
    // is dangerous
...
void DebugMesh::Generator::add_axis(glm::bvec3 axis, float length,
                                    float tip_length, bool rgb) {
  ...
  for (int dim = 0; dim < 3; ++dim) {
    if (axis[dim]) {
      ...
      if (rgb) next_attribute.color = colors[dim];
...
void DebugMesh::Generator::add_vertex(const glm::vec3& position) {
  ...
  vertices.last().position =
      transform_point(transformations.top(), vertices.last().position);
}
...
void DebugMesh::Generator::add_cylinder(float radius, float length,
                                        int nPoints) {
  push_matrix(glm::vec3(0, 0, 0.5f) * length, glm::vec3(0, 0, 1),
              glm::vec3(1, 0, 0));
  add_circle(radius, nPoints);
  pop_matrix();
...
void DebugMesh::Generator::add_rect(const glm::vec2& min,
                                    const glm::vec2& max) {
  begin_strip(CLOSE);
...
void DebugMesh::Generator::add_cube(const glm::vec3& min,
                                    const glm::vec3& max) {
...
void DebugMesh::Generator::add_arrow(float length, float tip_length) {
  ...
  for (const glm::vec2& dir : {glm::vec2(-1, -1), glm::vec2(-1, 1),
...
void DebugMesh::Generator::add_arrow(glm::vec3 origin, glm::vec3 tip,
                                     float tip_length) {
...
void DebugMesh::Generator::push_matrix(const glm::vec3& position,
                                       bool multiply) {
...
void DebugMesh::Generator::push_matrix(const glm::vec3& position,
                                       const glm::vec3& normal, bool multiply) {
  push_matrix(position, normal, find_best_perpendicular(normal), multiply);
}

void DebugMesh::Generator::push_matrix(const glm::vec3& position,
                                       const glm::vec3& normal,
                                       const glm::vec3& firstPointDirection,
                                       bool multiply) {
  glm::mat4 matrix =
      glm::mat4(glm::vec4(firstPointDirection, 0),
                glm::vec4(glm::cross(normal, firstPointDirection), 0),
                glm::vec4(normal, 0), glm::vec4(position, 1));
...
void DebugMesh::Generator::pop_matrix() {
  ...
  if (transformations.size() == 0) transformations.push(glm::mat4(1));
}
