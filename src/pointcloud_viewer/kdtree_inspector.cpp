...
bool KdTreeInspector::canBuildKdTree() const { return m_canBuildKdTree; }
...
KdTreeInspector::KdTreeInspector(QWidget* window) : window(window) {
  QSettings settings;
  setAutoBuildKdTreeAfterLoading(
      settings.value("Import/autoBuildKdTreeAfterLoading", false).toBool());
}

KdTreeInspector::~KdTreeInspector() {
  QSettings settings;
  settings.setValue("Import/autoBuildKdTreeAfterLoading",
                    autoBuildKdTreeAfterLoading());
}
...
void KdTreeInspector::handle_new_point_cloud(
...
void KdTreeInspector::kd_tree_inspection_move_to_root() {
  if (this->point_cloud == nullptr || !this->point_cloud->has_build_kdtree()) {
    kd_tree_inspection_changed(
        point_cloud ? point_cloud->aabb : aabb_t::invalid(),
        glm::vec3(INFINITY), aabb_t::invalid());
    return;
  }

  set_current_point_for_the_kd_tree_inspection(
      point_cloud->kdtree_index.root_point());
}

void KdTreeInspector::kd_tree_inspection_move_to_parent() {
  set_current_point_for_the_kd_tree_inspection(
      point_cloud->kdtree_index.parent_of(kd_tree_inspection_current_point));
}
...
void KdTreeInspector::kd_tree_inspection_move_to_subtree() {
  ...
  std::pair<size_t, size_t> children =
      point_cloud->kdtree_index.children_of(kd_tree_inspection_current_point);

  set_current_point_for_the_kd_tree_inspection(
      _left_selected ? children.first : children.second);
}
...
void KdTreeInspector::setAutoBuildKdTreeAfterLoading(
    bool autoBuildKdTreeAfterLoading) {
  if (m_autoBuildKdTreeAfterLoading == autoBuildKdTreeAfterLoading) return;
...
void KdTreeInspector::setCanBuildKdTree(bool canBuildKdTree) {
  if (m_canBuildKdTree == canBuildKdTree) return;
...
void KdTreeInspector::setHasKdTreeAvailable(bool hasKdTreeAvailable) {
  if (m_hasKdTreeAvailable == hasKdTreeAvailable) return;

  m_hasKdTreeAvailable = hasKdTreeAvailable;

  if (!hasKdTreeAvailable) kd_tree_inspection_move_to_root();

  emit hasKdTreeAvailableChanged(m_hasKdTreeAvailable);
}

void KdTreeInspector::set_current_point_for_the_kd_tree_inspection(
...
void KdTreeInspector::update_kd_tree_inspection() {
  ...
  glm::vec3 point = point_cloud->kdtree_index.point_coordinate(
      kd_tree_inspection_current_point, point_cloud->coordinate_color.data(),
      PointCloud::stride);
  std::pair<aabb_t, aabb_t> aabbs = point_cloud->kdtree_index.aabbs_split_by(
      kd_tree_inspection_current_point, point_cloud->coordinate_color.data(),
      PointCloud::stride);

  if (!this->_left_selected) aabbs = std::make_pair(aabbs.second, aabbs.first);
