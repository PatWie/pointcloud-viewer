...
FlythroughFile FlythroughFile::import_from_file(QString filepath) {
  ...
  if (file.read(reinterpret_cast<char*>(&result.header), sizeof(header_t)) !=
      sizeof(header_t))
  ...
  const int64_t keypoint_frames_size_bytes =
      sizeof(frame_t) * uint32_t(result.header.num_keypoints);

  if (file.size() - int64_t(sizeof(header_t)) != keypoint_frames_size_bytes)
    throw QString("Invalid file!\n(Number keypoints doesn't match filesize)");

  if (result.header.num_keypoints > 65535)  // AN upper limit just to have an
                                            // upper limit. Not tested. YOu can
                                            // pic a different one if needed
    throw QString("Invalid file!\n(Too many keypoints)");

  result.keypoint_frames.resize(result.header.num_keypoints);

  if (keypoint_frames_size_bytes !=
      file.read(reinterpret_cast<char*>(result.keypoint_frames.data()),
                keypoint_frames_size_bytes))
...
void FlythroughFile::export_to_file(QString filepath) {
  if (!filepath.endsWith(".camera_path")) filepath += ".camera_path";
  ...
  file.write(reinterpret_cast<const char*>(this->keypoint_frames.data()),
             sizeof(frame_t) * uint32_t(keypoint_frames.length()));
}
