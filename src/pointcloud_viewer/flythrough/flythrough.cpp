...
Flythrough::Flythrough(const Flythrough& flythrough)
    : _keypoints(flythrough._keypoints),
      interpolation_implementation(create_interpolation_implementation_for_enum(
          interpolation_t(flythrough.m_interpolation))),
...
Flythrough::Flythrough() {
  interpolation_implementation =
      QSharedPointer<Interpolation>(new LinearInterpolation(&this->_keypoints));
...
double Flythrough::animationDuration() const { return m_animationDuration; }

double Flythrough::cameraVelocity() const { return m_cameraVelocity; }

double Flythrough::pathLength() const { return m_pathLength; }

frame_t Flythrough::camera_position_for_time(double time,
                                             frame_t fallback) const {
  if (_keypoints.isEmpty()) return fallback;

  if (time < 0.) return _keypoints.first().frame;
  if (time > animationDuration()) return _keypoints.last().frame;
...
bool Flythrough::canPlay() const { return m_canPlay; }

int Flythrough::interpolation() const { return m_interpolation; }
...
void Flythrough::setAnimationDuration(double animationDuration) {
  ...
  if (qFuzzyCompare(m_animationDuration, animationDuration)) return;
...
void Flythrough::setCameraVelocity(double cameraVelocity) {
  ...
  if (qFuzzyCompare(m_cameraVelocity, cameraVelocity)) return;
...
QVariant Flythrough::headerData(int section, Qt::Orientation orientation,
                                int role) const {
...
Flythrough::create_interpolation_implementation_for_enum(
    Flythrough::interpolation_t interpolation) const {
...
void Flythrough::_init_connections() {
  connect(this, &Flythrough::animationDurationChanged, this,
          &Flythrough::updateCameraVelocits);
  connect(this, &Flythrough::cameraVelocityChanged, this,
          &Flythrough::updateAnimationDuration);
  connect(this, &Flythrough::pathLengthChanged, this,
          &Flythrough::updateAnimationDuration);
  ...
  connect(this, &Flythrough::animationDurationChanged, this, [this]() {
    playback._animationDuration = this->m_animationDuration;
  });
  connect(&playback, &Playback::request_next_frame, this,
          &Flythrough::newCameraPosition);
...
void Flythrough::setPathLength(double pathLength) {
  ...
  if (qFuzzyCompare(m_pathLength, pathLength)) return;
...
void Flythrough::newCameraPosition(double time) {
  ...
  if (Q_LIKELY(new_frame.scale_factor > 0.f)) set_new_camera_frame(new_frame);
}
...
void Flythrough::updateCameraVelocits() {
  if (pathLength() > 1) setCameraVelocity(pathLength() / animationDuration());
}

void Flythrough::updateAnimationDuration() {
  if (pathLength() > 1) setAnimationDuration(pathLength() / cameraVelocity());
}

void Flythrough::setCanPlay(bool canPlay) {
  if (m_canPlay == canPlay) return;
