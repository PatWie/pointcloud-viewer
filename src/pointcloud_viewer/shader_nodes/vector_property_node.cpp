...
VectorPropertyNode::VectorPropertyNode(
    QStringList supportedPropertyNames, QStringList missingPropertyNames,
    QMap<QString, property_type_t> property_base_types, QPixmap warning_icon)
  ...
  auto add_row = [vbox, this, &supportedPropertyNames, &missingPropertyNames](
      QComboBox* combobox, QLabel* warning_widget,
      void (VectorPropertyNode::*changed_property)(QString)) {
    ...
    for (QString name : supportedPropertyNames) combobox->addItem(name);
    for (QString name : missingPropertyNames) combobox->addItem(name);
  ...
  if (supportedPropertyNames.contains("x")) x_combobox->setCurrentText("x");
  if (supportedPropertyNames.contains("y")) y_combobox->setCurrentText("y");
  if (supportedPropertyNames.contains("z")) z_combobox->setCurrentText("z");
}
...
QString VectorPropertyNode::caption() const { return "VectorProperty"; }

QString VectorPropertyNode::name() const { return "VectorProperty"; }
...
QtNodes::NodeDataType VectorPropertyNode::dataType(QtNodes::PortType,
                                                   QtNodes::PortIndex) const {
  return Value().type();
}

void VectorPropertyNode::setInData(std::shared_ptr<QtNodes::NodeData> nodeData,
...
std::shared_ptr<QtNodes::NodeData> VectorPropertyNode::outData(
    QtNodes::PortIndex portIndex) {
  Q_ASSERT(portIndex == 0);
  return _vector_property;
}

void VectorPropertyNode::set_properties(const QString& nameX,
                                        const QString& nameY,
                                        const QString& nameZ) {
  auto set_property = [this](QComboBox* comboBox, QString name) {
    if (supportedPropertyNames.contains(name) ||
        missingPropertyNames.contains(name))
...
QWidget* VectorPropertyNode::embeddedWidget() { return _root_widget; }
...
void VectorPropertyNode::changedProperty() {
  value_type_t type =
      to_vector(result_type(_x_property->value_type, _y_property->value_type,
                            _z_property->value_type));

  _vector_property = std::make_shared<Value>(QString("%0(%1, %2, %3)")
                                                 .arg(format(type))
                                                 .arg(_x_property->expression)
                                                 .arg(_y_property->expression)
                                                 .arg(_z_property->expression),
                                             type);
  dataUpdated(0);
}
