...
class VectorPropertyNode final : public QtNodes::NodeDataModel {
 public:
  VectorPropertyNode(QStringList supportedPropertyNames,
  ...
  QVector<QString> vector_properties_names() const {
    return QVector<QString>({x_combobox->currentText(),
                             y_combobox->currentText(),
                             z_combobox->currentText()});
  }
  ...
  void set_properties(const QString& nameX, const QString& nameY,
                      const QString& nameZ);
 ...
 private:
  ...
  QStringList missingPropertyNames;
  std::shared_ptr<Value> _x_property =
      std::make_shared<Value>("0", VALUE_TYPE::INT);
  std::shared_ptr<Value> _y_property =
      std::make_shared<Value>("0", VALUE_TYPE::INT);
  std::shared_ptr<Value> _z_property =
      std::make_shared<Value>("0", VALUE_TYPE::INT);
  std::shared_ptr<Value> _vector_property =
      std::make_shared<Value>("ivec3(0)", VALUE_TYPE::IVEC3);
