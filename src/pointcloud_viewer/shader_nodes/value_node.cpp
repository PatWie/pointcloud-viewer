...
ValueNode::ValueNode() {
  ...
  for (value_type_t value_type : all_value_types)
    _combobox_type->addItem(format(value_type),
                            QVariant::fromValue(value_type));

  _content_box = new QLineEdit();

  set_value("vec3(0, 0, 0)", VALUE_TYPE::VEC3);

  connect(_content_box, static_cast<void (QLineEdit::*)(const QString&)>(
                            &QLineEdit::textChanged),
          this, &ValueNode::update_value_expression);
  connect(_combobox_type, static_cast<void (QComboBox::*)(int)>(
                              &QComboBox::currentIndexChanged),
          this, &ValueNode::update_value_type);
...
void ValueNode::restore(const QJsonObject& jsonObject) {
  ...
  set_value(jsonObject["value_expression"].toString(),
            value_type_from_string(jsonObject["value_type"].toString(),
                                   /*fallback*/ VALUE_TYPE::DVEC3));
}
...
void ValueNode::set_value(QString value, value_type_t value_type) {
  ...
  if (index >= 0) _combobox_type->setCurrentIndex(index);
  if (value != _content_box->text()) _content_box->setText(value);
  dataUpdated(0);
}
...
QString ValueNode::portCaption(QtNodes::PortType portType,
...
bool ValueNode::portCaptionVisible(QtNodes::PortType portType,
                                   QtNodes::PortIndex portIndex) const {
  Q_ASSERT(portType == QtNodes::PortType::Out);
  Q_ASSERT(portIndex == 0);
...
QtNodes::NodeDataType ValueNode::dataType(QtNodes::PortType,
                                          QtNodes::PortIndex) const {
  return Value().type();
}

void ValueNode::setInData(std::shared_ptr<QtNodes::NodeData> nodeData,
                          QtNodes::PortIndex portIndex) {
  Q_UNUSED(nodeData);
  Q_UNUSED(portIndex);
}

std::shared_ptr<QtNodes::NodeData> ValueNode::outData(
    QtNodes::PortIndex portIndex) {
  Q_ASSERT(portIndex == 0);
...
QWidget* ValueNode::embeddedWidget() { return _root; }
...
void ValueNode::update_value_type() {
  set_value(value->expression,
            _combobox_type->currentData().value<value_type_t>());
}
