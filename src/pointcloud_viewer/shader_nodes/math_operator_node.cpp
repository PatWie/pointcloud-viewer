...
MathOperatorNode::MathOperatorNode() {
  ...
  connect(_combobox_op, &QComboBox::currentTextChanged, this,
          &MathOperatorNode::update_operator);
  update_operator();
}
...
void MathOperatorNode::set_operator(QString op) {
  ...
  if (index >= 0) _combobox_op->setCurrentIndex(index);
}

QString MathOperatorNode::caption() const { return "Operator"; }

QString MathOperatorNode::name() const { return "Operator"; }
...
QString MathOperatorNode::portCaption(QtNodes::PortType portType,
...
bool MathOperatorNode::portCaptionVisible(QtNodes::PortType portType,
                                          QtNodes::PortIndex portIndex) const {
  switch (portType) {
    case QtNodes::PortType::In:
...
QtNodes::NodeDataType MathOperatorNode::dataType(QtNodes::PortType,
                                                 QtNodes::PortIndex) const {
  return Value().type();
}

void MathOperatorNode::setInData(std::shared_ptr<QtNodes::NodeData> nodeData,
...
std::shared_ptr<QtNodes::NodeData> MathOperatorNode::outData(
    QtNodes::PortIndex portIndex) {
...
QWidget* MathOperatorNode::embeddedWidget() { return _combobox_op; }

void MathOperatorNode::update_result() {
  value_type_t result_type = ::result_type(x->value_type, y->value_type);
  result = std::make_shared<Value>(QString("(%0 %2 %1)")
                                       .arg(x->expression)
                                       .arg(y->expression)
                                       .arg(operator_symbol),
                                   result_type);
