...
PropertyNode::PropertyNode(QStringList supportedPropertyNames,
  ...
  connect(_combobox, &QComboBox::currentTextChanged, this,
          &PropertyNode::changedProperty);

  for (QString name : supportedPropertyNames) _combobox->addItem(name);
  for (QString name : missingPropertyNames) _combobox->addItem(name);
}
...
QString PropertyNode::property_name() const { return _combobox->currentText(); }

QString PropertyNode::caption() const { return "Property"; }

QString PropertyNode::name() const { return "Property"; }
...
QtNodes::NodeDataType PropertyNode::dataType(QtNodes::PortType,
                                             QtNodes::PortIndex) const {
  return Value().type();
}

void PropertyNode::setInData(std::shared_ptr<QtNodes::NodeData> nodeData,
...
std::shared_ptr<QtNodes::NodeData> PropertyNode::outData(
    QtNodes::PortIndex portIndex) {
  Q_ASSERT(portIndex == 0);
  return _property;
}

QWidget* PropertyNode::embeddedWidget() { return _hbox_widget; }
