...
void MainWindow::handleApplicationArguments() {
  ...
  for (int argument_index = 1; argument_index < arguments.length();
       ++argument_index) {
    ...
    if (argument == "--non-interactive") {
      ...
      qDebug() << "Usage: pointcloud_viewer [ARGUMENTS]\n"
                  "\n"
                  "--help               Show this helptext and exits "
                  "immediately                   \n"
                  "--non-interactive    Hides the main window an immediately "
                  "starts rendering. Used\n"
                  "                     for using this software as "
                  "command-line tool.              \n"
                  "                     (Import and render dialog will appear "
                  "nevertheless but     \n"
                  ...
                  "                    \n"
                  "--first_index <INTEGER>  The first index used for the first "
                  "rendered image      \n"
                  "                     filename\n";
      std::exit(0);
    } else {
