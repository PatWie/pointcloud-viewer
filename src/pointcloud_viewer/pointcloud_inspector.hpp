...
class PointCloudInspector final : public QObject {
  Q_OBJECT
  Q_PROPERTY(
      double pointSelectionHighlightRadius READ pointSelectionHighlightRadius
          WRITE setPointSelectionHighlightRadius NOTIFY
              pointSelectionHighlightRadiusChanged)
  Q_PROPERTY(bool hasSelectedPoint READ hasSelectedPoint NOTIFY
                 hasSelectedPointChanged)
  Q_PROPERTY(int pickRadius READ pickRadius WRITE setPickRadius NOTIFY
                 pickRadiusChanged)
 public:
 ...
 public slots:
  ...
  PointCloud::vertex_t get_selected_point(
      PointCloud::vertex_t fallback = PointCloud::vertex_t{
          glm::vec3(0), glm::u8vec3(255, 0, 255)}) const;

 signals:
  void deselect_picked_point();
  void selected_point(glm::vec3 coordinate, glm::u8vec3 color,
                      PointCloud::UserData user_data);

  void pointSelectionHighlightRadiusChanged(
