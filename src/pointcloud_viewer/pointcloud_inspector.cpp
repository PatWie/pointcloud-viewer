...
PointCloudInspector::PointCloudInspector(Viewport* viewport)
    : viewport(*viewport) {
  QSettings settings;
  m_pointSelectionHighlightRadius =
      settings.value("UI/SelectionHighlightRadius", 0.5).toDouble();
}

PointCloudInspector::~PointCloudInspector() {
  QSettings settings;
  settings.setValue("UI/SelectionHighlightRadius",
                    m_pointSelectionHighlightRadius);
}
...
int PointCloudInspector::pickRadius() const { return m_pickRadius; }
...
void PointCloudInspector::handle_new_point_cloud(
...
void PointCloudInspector::pick_point(glm::ivec2 pixel) {
  ...
  if (!point_cloud->has_build_kdtree()) {
    QMessageBox msg_box(QMessageBox::Information, "No KD-Tree built",
                        "In order to be able to pick points, a KD-Tree must "
                        "have been built.\n\nBuild the KD-Tree now?",
                        QMessageBox::Yes | QMessageBox::No, &viewport);
  ...
  glm::vec2 screenspace_point =
      Camera::pixel_to_screenspace(pixel, viewport_size);

  ray_t center_ray =
      viewport.navigation.camera.ray_for_screenspace_point(screenspace_point);
  ray_t side_ray = viewport.navigation.camera.ray_for_screenspace_point(
      Camera::pixel_to_screenspace(pixel + glm::ivec2(pick_radius, 0),
                                   viewport_size));
...
void PointCloudInspector::update() {
  if (hasSelectedPoint()) {
    ...
    viewport.visualization().select_picked_point(vertex.coordinate,
                                                 vertex.color);
...
void PointCloudInspector::setPointSelectionHighlightRadius(
    double pointSelectionHighlightRadius) {
  qWarning("Floating point comparison needs context sanity check");
  if (qFuzzyCompare(m_pointSelectionHighlightRadius,
                    pointSelectionHighlightRadius))
    return;
...
void PointCloudInspector::setPickRadius(int pickRadius) {
  if (m_pickRadius == pickRadius) return;

  m_pickRadius = pickRadius;
  emit pickRadiusChanged(m_pickRadius);
}

PointCloud::vertex_t PointCloudInspector::get_selected_point(
    PointCloud::vertex_t fallback) const {
  if (hasSelectedPoint())
