...
Visualization::Visualization()
    : world_axis(DebugMesh::axis()),
      ...
      camera_path(
          DebugMesh::path(0, [](int) -> frame_t { Q_UNREACHABLE(); }, -1)) {}

void Visualization::render() {
  if (!settings.enable_any_visualizations) return;

  debug_mesh_renderer.begin();
  if (settings.enable_grid) debug_mesh_renderer.render(world_grid);
  if (settings.enable_axis) debug_mesh_renderer.render(world_axis);
  if (settings.enable_turntable_center)
    debug_mesh_renderer.render(turntable_origin);
  if (settings.enable_trackball) debug_mesh_renderer.render(trackball);
  if (settings.enable_camera_path) debug_mesh_renderer.render(camera_path);
  if (settings.enable_picked_cone) debug_mesh_renderer.render(picked_cone);
...
void Visualization::set_path(const QVector<keypoint_t>& keypoints,
                             int selected_point) {
  camera_path = DebugMesh::path(
      keypoints.length(), [&keypoints](int i) { return keypoints[i].frame; },
      selected_point);
}

void Visualization::set_kdtree_as_aabb(aabb_t highlighted_aabb,
                                       glm::vec3 separator_point,
                                       aabb_t other_aabb) {
  glm::vec3 normal_color = glm::vec3(1, 0.5, 0);
  glm::vec3 highlight_color = glm::vec3(1, 1, 0.2);

  kdtree_current_point =
      DebugMesh::turntable_point(separator_point, 0.25f, normal_color);
...
void Visualization::select_picked_point(glm::vec3 coordinate,
                                        glm::u8vec3 color) {
...
void Visualization::draw_overlay(QPainter& painter, const Camera& camera,
                                 int pointSize, glm::ivec2 viewport_size) {
  if (this->has_selected_point) {
    const glm::mat4 view_perspective_matrix = camera.view_perspective_matrix();
    const glm::vec3 selected_point = transform_point(
        view_perspective_matrix, this->selected_point_coordinate);

    if (selected_point.z > 1.f) return;

    glm::ivec2 selected_point_pixel =
        glm::ivec2(glm::round(Camera::screenspace_to_pixel(
            Camera::clipspace_screenspace(selected_point), viewport_size)));

    const bool is_bright_color =
        (selected_point_color.x + selected_point_color.y +
         selected_point_color.z) /
            2 >
        196;

    QRect selection_box = QRect(selected_point_pixel.x - (pointSize) / 2 - 2,
                                selected_point_pixel.y - (pointSize) / 2 - 2,
