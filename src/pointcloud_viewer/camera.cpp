...
glm::mat4 Camera::view_matrix() const { return frame.inverse().to_mat4(); }
...
ray_t Camera::ray_for_clipspace_point(glm::vec2 clipspace_point) const {
  const glm::mat4 inverse_perspective_matrix =
      glm::inverse(perspective_matrix());
  const glm::vec3 view_space = glm::normalize(transform_point(
      inverse_perspective_matrix, glm::vec3(clipspace_point, -1.f)));
...
glm::vec2 Camera::pixel_to_screenspace(glm::ivec2 screenspace_pixel,
                                       glm::ivec2 viewport_size) {
  return glm::vec2(screenspace_pixel) / glm::vec2(viewport_size - 1);
}

glm::vec2 Camera::clipspace_screenspace(glm::vec2 clipspace_point) {
  return clipspace_point * glm::vec2(.5f, -.5f) + glm::vec2(.5f, .5f);
}

glm::vec2 Camera::screenspace_to_pixel(glm::vec2 screenspace_point,
                                       glm::ivec2 viewport_size) {
  return screenspace_point * glm::vec2(viewport_size) - 1.e-6f;
}
